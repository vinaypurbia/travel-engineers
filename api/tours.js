  ...
  try {
    ...
    const html = `<div style="font-family:sans-serif;max-width:520px;margin:0 auto">
      <div style="background:#d4850a;padding:24px 28px;border-radius:12px 12px 0 0">
        <h2 style="color:white;margin:0">New Tour Booking Request</h2>
      ...
      <div style="padding:24px 28px;background:white;border-radius:0 0 12px 12px;border:1px solid #eee">
        <table style="width:100%;border-collapse:collapse;font-size:14px">
          ...
          <tr style="background:#fafafa"><td style="padding:8px 0;color:#888">Pax</td><td style="padding:8px 0">${booking.pax}</td></tr>
          ...
          <tr><td style="padding:8px 0;color:#888">Package Price</td><td style="padding:8px 0;font-weight:600;color:#d4850a">Rs.${(booking.basePrice||0).toLocaleString("en-IN")}</td></tr>
          <tr style="background:#fafafa"><td style="padding:8px 0;color:#888">Advance</td><td style="padding:8px 0;font-weight:600">Rs.${(booking.tokenAmount||0).toLocaleString("en-IN")}</td></tr>
    ...
    await transporter.sendMail({
      ...
      subject: `New Tour Request - ${booking.tourTitle} by ${booking.customerName}`,
      html,
    });
  } catch(err) { console.error("Tour email error:", err.message); }
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const isBookings = req.query.bookings === "1";
    ...
    if (isBookings) {
      ...
      if (req.method === "POST") {
        ...
        sendTourEmail({ booking, agencyEmail: agency && agency.email ? agency.email : process.env.GMAIL_USER, agencyName: agency && agency.name ? agency.name : "Travel Engineers" }).catch(console.error);
        return res.json({ success: true, booking, whatsappUrl });
      }
      return res.status(405).json({ error: "Method not allowed" });
    }

    // ── TOURS (called as /api/tours) ──────────────────────────────────────────
    if (id) {
      if (req.method === "GET") {
