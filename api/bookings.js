...
async function sendBookingEmail({ booking, days, agencyEmail, agencyName }) {
  ...
  if (!gmailUser || !gmailPass) {
    console.log("Email skipped: GMAIL_USER or GMAIL_PASS not set in Vercel env vars");
    return { sent: false, reason: "No email credentials" };
  }
  try {
    ...
    console.log("Email sent to:", agencyEmail || gmailUser);
    return { sent: true };
  } catch (err) {
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      const msg = [`🛵 *New Booking Request!*`,``,`*Vehicle:* ${vehicleName || "—"}`,`*Customer:* ${customerName}`,`*Phone:* ${phone}`,`*Check-in:* ${fmt(checkIn)}`,`*Check-out:* ${fmt(checkOut)}`,`*Duration:* ${days} day${days !== 1 ? "s" : ""}`,`*Stay Address:* ${stayAddress || "—"}`,notes ? `*Notes:* ${notes}` : null,``,`Booking ID: ${booking._id}`].filter(Boolean).join("\n");
      ...
      sendBookingEmail({ booking, days, agencyEmail: agency?.email || process.env.GMAIL_USER, agencyName: agency?.name || "Travel Engineers" }).catch(console.error);
