...
function ImageUpload({ value, onChange, label = "Image" }) {
  ...
  const handleFile = async (e) => {
    ...
    try {
      ...
      else alert("Upload failed. Check Cloudinary settings.");
    } catch {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
        <input className="adm-input" value={value || ""} onChange={e => onChange(e.target.value)} placeholder="Paste URL or upload below" style={{ flex: 1 }} />
...
export default function App() {
  ...
  if (view === "login") return <LoginScreen loginInput={loginInput} setLoginInput={setLoginInput} loginError={loginError}
    onLogin={async () => {
      const res = await api.post("/auth/login", { password: loginInput });
    ...
    <div style={{ fontFamily: "'Lora', Georgia, serif", background: "#faf8f3", color: "#1a1a2e", minHeight: "100vh" }}>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, padding: "0 5%", height: 70, display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(10,22,40,0.95)", backdropFilter: "blur(12px)", borderBottom: "1px solid rgba(212,133,10,0.2)" }}>
        ...
        <div style={{ display: "flex", gap: 28, alignItems: "center" }}>
          ...
          <button onClick={() => setView("login")} style={{ background: "rgba(212,133,10,0.2)", border: "1px solid #d4850a", color: "#f0c060", padding: "7px 16px", borderRadius: 20, cursor: "pointer", fontFamily: "'DM Sans'", fontSize: 12, fontWeight: 600 }}>
            ADMIN
          </button>
        </div>
      </nav>
      ...
      <div style={{ background: "#0a1628", padding: "28px 5%", display: "flex", justifyContent: "space-around", flexWrap: "wrap", gap: 20 }}>
        {[["500+", "Happy Customers"], [villa.rooms?.length || 6, "Villa Rooms"], [rentals.length, "Vehicles"], ["24/7", "Support"]].map(([n, l]) => (
      ...
      <section id="sec-contact" style={{ background: "#0a1628", padding: "100px 5%", color: "white" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", textAlign: "center" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 24, marginBottom: 48 }}>
            {[
              { icon: "📞", label: "Call / WhatsApp", value: agency.phone, href: `tel:${agency.phone}` },
              { icon: "✉️", label: "Email", value: agency.email, href: `mailto:${agency.email}` },
              { icon: "📍", label: "Location", value: agency.address, href: "#" },
            ].map(c => (
    ...
    <div style={{ minHeight: "100vh", background: "#0d1b2e", fontFamily: "'DM Sans', sans-serif", color: "white" }}>
      ...
      <div style={{ background: "rgba(0,0,0,0.3)", borderBottom: "1px solid rgba(240,192,96,0.15)", padding: "0 32px", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          {saved && <span style={{ color: "#4ade80", fontSize: 13, display: "flex", alignItems: "center", gap: 6 }}><Icon name="check" size={16} /> Saved to MongoDB!</span>}
...
function AgencyEditor({ data, api, showSaved }) {
  ...
  return (
    <div>
      ...
      <button onClick={save} style={{ background: "linear-gradient(135deg,#d4850a,#f0c060)", color: "#1a1a2e", border: "none", padding: "12px 32px", borderRadius: 10, fontWeight: 700, fontSize: 14, cursor: "pointer" }}>Save to MongoDB</button>
    </div>
  );
}
...
function RentalsEditor({ data, api, reload, showSaved }) {
  ...
  const deleteRental = async (id) => {
    if (window.confirm("Delete this vehicle?")) { await api.delete(`/rentals/${id}`); await reload(); }
  };
      ...
      {(editId || adding) && form && (
        <div className="adm-card" style={{ marginBottom: 28, border: "1px solid rgba(212,133,10,0.3)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
            {[["Vehicle Name", "name"], ["Price (e.g. ₹350)", "price"], ["Period", "period"], ["Tag (optional)", "tag"]].map(([l, k]) => (
              <div key={k}><label className="adm-label">{l}</label><input className="adm-input" value={form[k] || ""} onChange={e => set(k, e.target.value)} /></div>
            ))}
            ...
            <div style={{ gridColumn: "1 / -1" }}>
              <ImageUpload label="Vehicle Image" value={form.image} onChange={v => set("image", v)} />
            </div>
          </div>
          <div style={{ marginBottom: 16 }}>
            ...
            {(form.features || []).map((f, i) => (
              <div key={i} style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                <input className="adm-input" value={f} onChange={e => setFeature(i, e.target.value)} />
                <button onClick={() => setForm(f => ({ ...f, features: f.features.filter((_, j) => j !== i) }))} style={{ background: "rgba(255,80,80,0.15)", border: "none", color: "#ff6b6b", borderRadius: 8, padding: "0 12px", cursor: "pointer" }}>×</button>
              </div>
            ))}
            <button onClick={() => setForm(f => ({ ...f, features: [...f.features, ""] }))} style={{ background: "transparent", border: "1px dashed rgba(255,255,255,0.2)", color: "rgba(255,255,255,0.4)", padding: "8px 16px", borderRadius: 8, cursor: "pointer", fontSize: 13 }}>+ Add Feature</button>
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={saveRental} style={{ background: "linear-gradient(135deg,#d4850a,#f0c060)", color: "#1a1a2e", border: "none", padding: "10px 24px", borderRadius: 8, fontWeight: 700, cursor: "pointer" }}>Save to MongoDB</button>
        ...
        {data.rentals.map(r => (
          <div key={r._id} className="adm-card" style={{ display: "flex", gap: 16, alignItems: "center" }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexShrink: 0 }}>
              <button onClick={() => toggleAvail(r)} style={{ fontSize: 12, padding: "6px 14px", borderRadius: 20, border: "none", cursor: "pointer", background: r.available ? "rgba(74,222,128,0.15)" : "rgba(255,80,80,0.15)", color: r.available ? "#4ade80" : "#ff6b6b", fontWeight: 600 }}>
    ...
    <div>
      ...
      <div className="adm-card" style={{ marginBottom: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          ...
          <button onClick={() => setForm(f => ({ ...f, amenities: [...f.amenities, { icon: "🌟", label: "New Amenity" }] }))} style={{ background: "rgba(212,133,10,0.15)", border: "1px solid rgba(212,133,10,0.3)", color: "#f0c060", padding: "6px 14px", borderRadius: 8, cursor: "pointer", fontSize: 13 }}>+ Add</button>
        </div>
      ...
      <button onClick={save} style={{ background: "linear-gradient(135deg,#d4850a,#f0c060)", color: "#1a1a2e", border: "none", padding: "12px 32px", borderRadius: 10, fontWeight: 700, fontSize: 14, cursor: "pointer" }}>Save Villa to MongoDB</button>
    </div>
  );
}
